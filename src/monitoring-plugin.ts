...
import {
  ...
  runReflectionCycle,
  updateReflectionArtifactStatus,
} from "@phantasy/agent/plugin-runtime";
import { MONITORING_PLUGIN_METADATA } from "./plugin-metadata";
...
export default class MonitoringPlugin extends BasePlugin {
  name = MONITORING_PLUGIN_METADATA.name;
  version = MONITORING_PLUGIN_METADATA.version;
  description = MONITORING_PLUGIN_METADATA.description;
  protected displayName = MONITORING_PLUGIN_METADATA.displayName;
  protected category = MONITORING_PLUGIN_METADATA.category;
  protected tags = [...MONITORING_PLUGIN_METADATA.tags];
  protected permissions: string[] = [...MONITORING_PLUGIN_METADATA.permissions];
  protected workspace = MONITORING_PLUGIN_METADATA.workspace;
  protected extensionKind = MONITORING_PLUGIN_METADATA.extensionKind;
  protected configSchema = MONITORING_PLUGIN_METADATA.configSchema;
  protected adminSurface = MONITORING_PLUGIN_METADATA.adminSurface;
  ...
  async handleCustomEndpoint(
    ...
    try {
      ...
      const reflectionUpdateMatch = path.match(/^\/reflections\/([^/]+)$/);
      if (reflectionUpdateMatch && request.method === "PATCH") {
        const body = (await request.json().catch(() => ({}))) as {
          status?: string;
        };
        ...
        if (
          ...
          return jsonResponse(
            { error: "status must be reviewed, promoted, or archived" },
            400,
          );
        }
        ...
        const { agentId, workspacePath } =
          await resolveMonitoringAgentContext(request);
        const artifact = await updateReflectionArtifactStatus({
          artifactIdOrSlug: decodeURIComponent(reflectionUpdateMatch[1]),
          status,
          workspaceRoot: workspacePath,
          agentId,
        });
        if (!artifact) {
          return jsonResponse({ error: "Reflection artifact not found" }, 404);
        }

        return jsonResponse({ agentId, artifact });
      }
